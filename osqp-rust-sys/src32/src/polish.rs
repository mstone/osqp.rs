...
unsafe extern "C" fn form_Ared(mut work: *mut OSQPWorkspace) -> c_int {
    let mut j: c_int = 0;
    let mut ptr: c_int = 0;
    let mut Ared_nnz: c_int = 0 as ::std::os::raw::c_int;
    (*(*work).pol).n_low = 0 as ::std::os::raw::c_int;
    (*(*work).pol).n_upp = 0 as ::std::os::raw::c_int;
    ...
    while j < (*(*work).data).m {
            ...
            < -*((*work).y).offset(j as isize)
        {
            ...
            *((*(*work).pol).A_to_Alow).offset(j as isize) = -(1 as ::std::os::raw::c_int);
        }
        j += 1;
    }
    j = 0 as ::std::os::raw::c_int;
    while j < (*(*work).data).m {
            ...
            < *((*work).y).offset(j as isize)
        {
            ...
            *((*(*work).pol).A_to_Aupp).offset(j as isize) = -(1 as ::std::os::raw::c_int);
        }
        j += 1;
    }
    if (*(*work).pol).n_low + (*(*work).pol).n_upp == 0 as ::std::os::raw::c_int {
        ...
        *fresh4 = csc_spalloc(
        ...
        int_vec_set_scalar(
            (*(*(*work).pol).Ared).p,
            0 as ::std::os::raw::c_int,
            (*(*work).data).n + 1 as ::std::os::raw::c_int,
        );
    ...
    *fresh5 = csc_spalloc(
    ...
    Ared_nnz = 0 as ::std::os::raw::c_int;
    j = 0 as ::std::os::raw::c_int;
    while j < (*(*work).data).n {
        *((*(*(*work).pol).Ared).p).offset(j as isize) = Ared_nnz;
        ptr = *((*(*(*work).data).A).p).offset(j as isize);
        while ptr < *((*(*(*work).data).A).p).offset((j + 1 as ::std::os::raw::c_int) as isize) {
            if *((*(*work).pol).A_to_Alow)
...
unsafe extern "C" fn iterative_refinement(
    ...
    if (*(*work).settings).polish_refine_iter > 0 as ::std::os::raw::c_int {
        ...
        if rhs.is_null() {
            ...
            while i < (*(*work).settings).polish_refine_iter {
                ...
                mat_vec((*(*work).data).P, z, rhs, -(1 as ::std::os::raw::c_int));
                mat_tpose_vec(
                    (*(*work).data).P,
                    z,
                    rhs,
...
unsafe extern "C" fn get_ypol_from_yred(
    ...
    j = 0 as ::std::os::raw::c_int;
    while j < (*(*work).data).m {
        if *((*(*work).pol).A_to_Alow).offset(j as isize) != -(1 as ::std::os::raw::c_int) {
            *((*(*work).pol).y)
                .offset(
                    j as isize,
                ) = *yred
                .offset(*((*(*work).pol).A_to_Alow).offset(j as isize) as isize);
        } else if *((*(*work).pol).A_to_Aupp).offset(j as isize) != -(1 as ::std::os::raw::c_int) {
            *((*(*work).pol).y)
                .offset(
                    j as isize,
                ) = *yred
...
pub unsafe extern "C" fn polish(mut work: *mut OSQPWorkspace) -> c_int {
    ...
    osqp_tic((*work).timer);
    mred = form_Ared(work);
    if mred < 0 as ::std::os::raw::c_int {
        (*(*work).info).status_polish = -(1 as ::std::os::raw::c_int);
        return -(1 as ::std::os::raw::c_int);
    }
    ...
    rhs_red = malloc(
        (::std::mem::size_of::<c_float>() as ::std::os::raw::c_ulong)
            .wrapping_mul(((*(*work).data).n + mred) as ::std::os::raw::c_ulong),
    ) as *mut c_float;
    if rhs_red.is_null() {
        (*(*work).info).status_polish = -(1 as ::std::os::raw::c_int);
        csc_spfree((*(*work).pol).Ared);
    ...
    mat_vec((*(*work).data).A, (*(*work).pol).x, (*(*work).pol).z, 0 as ::std::os::raw::c_int);
    get_ypol_from_yred(work, pol_sol.offset((*(*work).data).n as isize));
    project_normalcone(work, (*(*work).pol).z, (*(*work).pol).y);
    update_info(work, 0 as ::std::os::raw::c_int, 1 as ::std::os::raw::c_int, 1 as ::std::os::raw::c_int);
    polish_successful = ((*(*work).pol).pri_res < (*(*work).info).pri_res
        ...
        || (*(*work).pol).dua_res < (*(*work).info).dua_res
            && (*(*work).info).pri_res < 1e-10f64) as ::std::os::raw::c_int;
