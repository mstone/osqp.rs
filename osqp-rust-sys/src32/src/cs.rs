...
pub unsafe extern "C" fn csc_spalloc(
    ...
    (*A).m = m;
    (*A).n = n;
    nzmax = if nzmax > 1 as ::std::os::raw::c_int { nzmax } else { 1 as ::std::os::raw::c_int };
    (*A).nzmax = nzmax;
    (*A).nz = if triplet != 0 { 0 as ::std::os::raw::c_int } else { -(1 as ::std::os::raw::c_int) };
    let ref mut fresh3 = (*A).p;
    *fresh3 = csc_malloc(
        if triplet != 0 { nzmax } else { n + 1 as ::std::os::raw::c_int },
...
pub unsafe extern "C" fn triplet_to_csc(
    ...
    C = csc_spalloc(
        ...
        n,
        nz,
        (Tx != OSQP_NULL as *mut c_float) as ::std::os::raw::c_int,
...
pub unsafe extern "C" fn csc_cumsum(
    ...
    let mut i: c_int = 0;
    let mut nz: c_int = 0 as ::std::os::raw::c_int;
...
pub unsafe extern "C" fn csc_symperm(
    ...
    C = csc_spalloc(
        ...
        *Ap.offset(n as isize),
        (values != 0 && !Ax.is_null()) as ::std::os::raw::c_int,
        0 as ::std::os::raw::c_int,
    );
    w = csc_calloc(n, ::std::mem::size_of::<c_int>() as ::std::os::raw::c_ulong as c_int)
        as *mut c_int;
...
pub unsafe extern "C" fn copy_csc_mat(mut A: *const csc) -> *mut csc {
    ...
    prea_int_vec_copy((*A).p, (*B).p, (*A).n + 1 as ::std::os::raw::c_int);
...
pub unsafe extern "C" fn csc_to_triu(mut M: *mut csc) -> *mut csc {
    ...
    let mut i: c_int = 0;
    let mut j: c_int = 0;
    let mut z_M: c_int = 0 as ::std::os::raw::c_int;
    if (*M).m != (*M).n {
        printf(
    ...
    nnzmaxM = nnzorigM + n;
    M_trip = csc_spalloc(n, n, nnzmaxM, 1 as ::std::os::raw::c_int, 1 as ::std::os::raw::c_int);
    if M_trip.is_null() {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<&[u8; 12], &[::std::os::raw::c_char; 12]>(b"csc_to_triu\0"))
