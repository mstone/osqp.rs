...
pub unsafe extern "C" fn osqp_setup(
    ...
    *fresh38 = malloc(
        ((*data).m as ::std::os::raw::c_ulong)
            .wrapping_mul(::std::mem::size_of::<c_int>() as ::std::os::raw::c_ulong),
    ) as *mut c_int;
    ...
    *fresh48 = calloc(
    ...
    (*(*work).info).status_polish = 0 as ::std::os::raw::c_int;
    ...
    (*work).first_run = 1 as ::std::os::raw::c_int;
    (*work).clear_update_time = 0 as ::std::os::raw::c_int;
    (*work).rho_update_from_solve = 0 as ::std::os::raw::c_int;
    (*(*work).info).rho_updates = 0 as ::std::os::raw::c_int;
...
pub unsafe extern "C" fn osqp_solve(mut work: *mut OSQPWorkspace) -> c_int {
    ...
    if (*work).clear_update_time == 1 as ::std::os::raw::c_int {
        (*(*work).info).update_time = 0.0f64;
    }
    (*work).rho_update_from_solve = 1 as ::std::os::raw::c_int;
    exitflag = 0 as ::std::os::raw::c_int;
    can_check_termination = 0 as ::std::os::raw::c_int;
    ...
    loop {
            ...
            && temp_run_time >= (*(*work).settings).time_limit
        {
            ...
            can_print = 0 as ::std::os::raw::c_int;
            current_block = 13125627826496529465;
            break;
        } else {
            can_check_termination = ((*(*work).settings).check_termination != 0
                && iter % (*(*work).settings).check_termination == 0 as ::std::os::raw::c_int)
                as ::std::os::raw::c_int;
            can_print = ((*(*work).settings).verbose != 0
                && (iter % PRINT_INTERVAL == 0 as ::std::os::raw::c_int
                    || iter == 1 as ::std::os::raw::c_int)) as ::std::os::raw::c_int;
            if can_check_termination != 0 || can_print != 0 {
                update_info(work, iter, compute_cost_function, 0 as ::std::os::raw::c_int);
                if can_print != 0 {
                    print_summary(work);
                }
                if can_check_termination != 0 {
                    if check_termination(work, 0 as ::std::os::raw::c_int) != 0 {
                        current_block = 13125627826496529465;
                        break;
                    }
                }
            ...
            if (*(*work).settings).adaptive_rho != 0
                && (*(*work).settings).adaptive_rho_interval != 0
                && iter % (*(*work).settings).adaptive_rho_interval == 0 as ::std::os::raw::c_int
            {
                if can_check_termination == 0 && can_print == 0 {
                    update_info(work, iter, compute_cost_function, 0 as ::std::os::raw::c_int);
                }
                if adapt_rho(work) != 0 {
                    ...
                    current_block = 6413470264264961813;
                    break;
                }
            }
            iter += 1;
    ...
    match current_block {
        13125627826496529465 => {
            if can_check_termination == 0 {
                ...
                check_termination(work, 0 as ::std::os::raw::c_int);
            }
            ...
            if (*(*work).info).status_val == OSQP_UNSOLVED {
                if check_termination(work, 1 as ::std::os::raw::c_int) == 0 {
            ...
            if (*work).first_run != 0 {
                (*work).first_run = 0 as ::std::os::raw::c_int;
            }
            (*work).clear_update_time = 1 as ::std::os::raw::c_int;
            (*work).rho_update_from_solve = 0 as ::std::os::raw::c_int;
            if (*(*work).settings).verbose != 0 {
...
pub unsafe extern "C" fn osqp_warm_start(
    ...
    if (*(*work).settings).warm_start == 0 {
        (*(*work).settings).warm_start = 1 as ::std::os::raw::c_int;
    }
    ...
    mat_vec((*(*work).data).A, (*work).x, (*work).z, 0 as ::std::os::raw::c_int);
    return 0 as ::std::os::raw::c_int;
}
#[no_mangle]
...
pub unsafe extern "C" fn osqp_update_P(
    ...
    reset_info((*work).info);
    if exitflag < 0 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
                &[u8; 14],
...
pub unsafe extern "C" fn osqp_update_rho(
    ...
    if (*work).rho_update_from_solve == 0 as ::std::os::raw::c_int {
        if (*work).clear_update_time == 1 as ::std::os::raw::c_int {
            (*work).clear_update_time = 0 as ::std::os::raw::c_int;
            (*(*work).info).update_time = 0.0f64;
        }
    ...
    i = 0 as ::std::os::raw::c_int;
    while i < (*(*work).data).m {
        if *((*work).constr_type).offset(i as isize) == 0 as ::std::os::raw::c_int {
            *((*work).rho_vec).offset(i as isize) = (*(*work).settings).rho;
            *((*work).rho_inv_vec).offset(i as isize) = 1.0f64 / (*(*work).settings).rho;
        } else if *((*work).constr_type).offset(i as isize) == 1 as ::std::os::raw::c_int {
            *((*work).rho_vec)
...
pub unsafe extern "C" fn osqp_update_max_iter(
    ...
    if max_iter_new <= 0 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
                &[u8; 21],
...
pub unsafe extern "C" fn osqp_update_warm_start(
    ...
    if warm_start_new != 0 as ::std::os::raw::c_int && warm_start_new != 1 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
                &[u8; 23],
...
pub unsafe extern "C" fn osqp_update_scaled_termination(
    ...
    if scaled_termination_new != 0 as ::std::os::raw::c_int
        && scaled_termination_new != 1 as ::std::os::raw::c_int
    {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
...
pub unsafe extern "C" fn osqp_update_check_termination(
    ...
    if check_termination_new < 0 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
                &[u8; 30],
...
pub unsafe extern "C" fn osqp_update_polish(
    ...
    if polish_new != 0 as ::std::os::raw::c_int && polish_new != 1 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
                &[u8; 19],
...
pub unsafe extern "C" fn osqp_update_polish_refine_iter(
    ...
    if polish_refine_iter_new < 0 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
                &[u8; 31],
...
pub unsafe extern "C" fn osqp_update_verbose(
    ...
    if verbose_new != 0 as ::std::os::raw::c_int && verbose_new != 1 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<
                &[u8; 20],
