...
pub unsafe extern "C" fn form_KKT(
    ...
    while j < (*A).n {
        ...
        while ptr < *((*A).p).offset((j + 1 as ::std::os::raw::c_int) as isize) {
            *((*KKT_trip).p)
    ...
    if PtoKKT.is_null() && AtoKKT.is_null() && param2toKKT.is_null() {
        if format == 0 as ::std::os::raw::c_int {
