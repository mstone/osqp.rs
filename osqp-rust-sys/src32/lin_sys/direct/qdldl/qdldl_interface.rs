...
unsafe extern "C" fn LDL_factor(
    ...
    if sum_Lnz < 0 as ::std::os::raw::c_int {
        printf(
        ...
        if sum_Lnz == -(1 as ::std::os::raw::c_int) {
            printf(
                b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
                (*::std::mem::transmute::<
                    &[u8; 11],
            ...
            printf(b"\n\0" as *const u8 as *const ::std::os::raw::c_char);
        } else if sum_Lnz == -(2 as ::std::os::raw::c_int) {
            printf(
                b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
                (*::std::mem::transmute::<
                    &[u8; 11],
    ...
    *fresh0 = malloc(
        (::std::mem::size_of::<c_int>() as ::std::os::raw::c_ulong)
            .wrapping_mul(sum_Lnz as ::std::os::raw::c_ulong),
    ) as *mut c_int;
    ...
    *fresh1 = malloc(
    ...
    if factor_status < 0 as ::std::os::raw::c_int {
        printf(
            b"ERROR in %s: \0" as *const u8 as *const ::std::os::raw::c_char,
            (*::std::mem::transmute::<&[u8; 11], &[::std::os::raw::c_char; 11]>(b"LDL_factor\0"))
                .as_ptr(),
...
unsafe extern "C" fn permute_KKT(
    ...
    amd_status = amd_order(
        (**KKT).n,
        (**KKT).p as *const ::std::os::raw::c_int,
        (**KKT).i as *const ::std::os::raw::c_int,
        (*p).P,
        0 as *mut c_float,
        info,
    );
    if amd_status < 0 as ::std::os::raw::c_int {
        free(info as *mut ::std::os::raw::c_void);
        return amd_status;
    }
    Pinv = csc_pinv((*p).P, (**KKT).n);
    if PtoKKT.is_null() && AtoKKT.is_null() && rhotoKKT.is_null() {
        KKT_temp = csc_symperm(*KKT, Pinv, OSQP_NULL as *mut c_int, 1 as ::std::os::raw::c_int);
    } else {
        KtoPKPt = malloc(
            (*((**KKT).p).offset((**KKT).n as isize) as ::std::os::raw::c_ulong)
                .wrapping_mul(::std::mem::size_of::<c_int>() as ::std::os::raw::c_ulong),
        ) as *mut c_int;
        KKT_temp = csc_symperm(*KKT, Pinv, KtoPKPt, 1 as ::std::os::raw::c_int);
        if !PtoKKT.is_null() {
...
pub unsafe extern "C" fn init_linsys_solver_qdldl(
    ...
    *fresh12 = malloc(
        (::std::mem::size_of::<c_float>() as ::std::os::raw::c_ulong)
            .wrapping_mul((*s).m as ::std::os::raw::c_ulong),
    ) as *mut c_float;
    let ref mut fresh13 = (*s).etree;
    *fresh13 = malloc(
        (n_plus_m as ::std::os::raw::c_ulong)
            .wrapping_mul(::std::mem::size_of::<QDLDL_int>() as ::std::os::raw::c_ulong),
    ) as *mut QDLDL_int;
    ...
    *fresh15 = malloc(
        ((n_plus_m + 1 as ::std::os::raw::c_int) as ::std::os::raw::c_ulong)
            .wrapping_mul(::std::mem::size_of::<QDLDL_int>() as ::std::os::raw::c_ulong),
    ) as *mut c_int;
    ...
    *fresh18 = malloc(
        (::std::mem::size_of::<QDLDL_int>() as ::std::os::raw::c_ulong)
            .wrapping_mul((3 as ::std::os::raw::c_int * n_plus_m) as ::std::os::raw::c_ulong),
    ) as *mut QDLDL_int;
    ...
    *fresh20 = malloc(
    ...
    if LDL_factor(KKT_temp, s, (*P).n) < 0 as ::std::os::raw::c_int {
        csc_spfree(KKT_temp);
        free_linsys_solver_qdldl(s);
        *sp = OSQP_NULL as *mut qdldl_solver;
        return OSQP_NONCVX_ERROR as ::std::os::raw::c_int;
    }
    if polish != 0 {
        csc_spfree(KKT_temp);
    } else {
