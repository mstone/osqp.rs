...
pub unsafe extern "C" fn check_termination(
    ...
    if prim_res_check != 0 && dual_res_check != 0 {
        if approximate != 0 {
            update_status((*work).info, OSQP_SOLVED_INACCURATE as c_int);
        } else {
        ...
        if approximate != 0 {
            ...
            update_status((*work).info, OSQP_PRIMAL_INFEASIBLE as c_int);
        }
        if (*(*work).settings).scaling != 0
            && (*(*work).settings).scaled_termination == 0
        {
        ...
        if approximate != 0 {
            ...
            update_status((*work).info, OSQP_DUAL_INFEASIBLE as c_int);
        }
        if (*(*work).settings).scaling != 0
            && (*(*work).settings).scaled_termination == 0
        {
