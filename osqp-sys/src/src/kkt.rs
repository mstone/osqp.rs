...
pub unsafe extern "C" fn form_KKT(
    ...
    let mut i: c_int = 0;
    let mut j: c_int = 0;
    let mut zKKT: c_int = 0 as libc::c_int as c_int;
    ...
    if !Pdiag_idx.is_null() {
        *Pdiag_idx = malloc(
            ((*P).m as libc::c_ulonglong)
                .wrapping_mul(
                    ::std::mem::size_of::<c_int>() as libc::c_ulong as libc::c_ulonglong,
                ) as libc::c_ulong,
        ) as *mut c_int;
        *Pdiag_n = 0 as libc::c_int as c_int;
    }
    j = 0 as libc::c_int as c_int;
    while j < (*P).n {
        if *((*P).p).offset(j as isize)
            == *((*P).p).offset((j + 1 as libc::c_int as libc::c_longlong) as isize)
        {
        ...
        while ptr < *((*P).p).offset((j + 1 as libc::c_int as libc::c_longlong) as isize)
        {
            ...
            *((*KKT_trip).x).offset(zKKT as isize) = *((*P).x).offset(ptr as isize);
            ...
            zKKT += 1;
            if i < j
                && ptr + 1 as libc::c_int as libc::c_longlong
                    == *((*P).p)
    ...
    if !Pdiag_idx.is_null() {
        *Pdiag_idx = realloc(
            *Pdiag_idx as *mut libc::c_void,
            (*Pdiag_n as libc::c_ulonglong)
                .wrapping_mul(
                    ::std::mem::size_of::<c_int>() as libc::c_ulong as libc::c_ulonglong,
                ) as libc::c_ulong,
        ) as *mut c_int;
    ...
    while j < (*A).n {
        ...
        while ptr < *((*A).p).offset((j + 1 as libc::c_int as libc::c_longlong) as isize)
        {
            *((*KKT_trip).p)
    ...
    if PtoKKT.is_null() && AtoKKT.is_null() && param2toKKT.is_null() {
        if format == 0 as libc::c_int as libc::c_longlong {
        ...
        if KKT_TtoC.is_null() {
            csc_spfree(KKT_trip);
            free(*Pdiag_idx as *mut libc::c_void);
            return OSQP_NULL as *mut csc;
        }
        if format == 0 as libc::c_int as libc::c_longlong {
        ...
        free(KKT_TtoC as *mut libc::c_void);
    }
    csc_spfree(KKT_trip);
    return KKT;
}
